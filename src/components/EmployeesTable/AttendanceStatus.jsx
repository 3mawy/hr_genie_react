...
const AttendanceStatus = ({employee, date, attendance_status}) => {
    ...
    const [updatedStatus, setUpdatedStatus] = useState(attendance_status); // Store updated status

    const handleAttendanceAdd = async (attendanceStatus) => {
        await addAttendance({employee, date, attendanceStatus});
        setUpdatedStatus(attendanceStatus);
    };
        ...
        return (
            <div
                ...
                <div className={`flex gap-2 items-center`}>
                    {(isLoadingAdd && !((attendance_status === 'present') || (attendance_status === 'absent')))?
                        <>
                            <div
